...
const ExpenseTable = ({ expenses,setExpenses,setExpense,setEditingrowId }) => {
  ...
  const totalAmount = filteredData.reduce((acc, curr) => acc + parseInt(curr.amount), 0);

  const [menuPosition,setMenuPosition]=useState({})

  const  [rowId,setRowId]=useState(null)
  const  [sortCallback,setSortCallback]=useState(()=>()=>{}) //we need to return a function from callback to avoid undefined
  ...
  function handleSelect(e) {
   ...
   return (
    <>
    <ContextMenu menuPosition={menuPosition} rowId={rowId} setExpenses={setExpenses} expenses={expenses} setMenuPosition={setMenuPosition} setExpense={setExpense}   setEditingrowId={setEditingrowId}
/>
